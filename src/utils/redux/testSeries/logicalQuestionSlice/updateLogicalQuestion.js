...
export const update_logical_que = createAsyncThunk("update_logical_que", async ({ question_id, answer_id, question, answer }, thunkAPI) => {
    try {
        ...
        const response = await fetch(`${process.env.REACT_APP_BACKEND_URL}/update-logical-and-subjective-question?question_id=${question_id}&answer_id=${answer_id}`, requestOptions)
        if (!response.ok) {
